...
01_ENV_Check
    ...
    \    Do SSH CMD    ${ip}    ${USERNAME}    ${PASSWORD}    sed \ -i.bak 's/Timeout 300/Timeout 900/' \ /etc/apache2/apache2.conf
    \    Do SSH CMD    ${ip}    ${USERNAME}    ${PASSWORD}    sed \ -i 's/KeepAliveTimeout 5/KeepAliveTimeout 600/' \ /etc/apache2/apache2.conf; /etc/init.d/apache2 restart
    log    Check Disk
